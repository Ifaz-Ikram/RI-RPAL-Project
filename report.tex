...
The interpreter is structured as a sequential pipeline where each stage transforms the program representation before passing it to the next.
...
\begin{tikzpicture}[
    ...
    arrow/.style={-{Stealth[length=2.6mm]}, thick, gray!75},
    artifact/.style={font=\itshape\scriptsize, text=gray!80}
]
\node[stage] (source) {Source\\File};
\node[stage, right=1.55cm of source] (lexer) {Lexer};
\node[stage, right=1.9cm of lexer] (parser) {Parser};
\node[stage, below=1.25cm of parser] (std) {Standardizer};
\node[stage, left=1.15cm of std] (cse) {CSE\\Machine};
\node[stage, left=1.15cm of cse] (output) {Output};

\draw[arrow] (source) -- (lexer);
\draw[arrow] (lexer) -- node[artifact, above=4pt] {Token Stream} (parser);
\draw[arrow] (parser) -- node[artifact, right] {AST} (std);
\draw[arrow] (std) -- node[artifact, above] {ST} (cse);
\draw[arrow] (cse) -- (output);
\end{tikzpicture}
\caption{Interpreter Pipeline}
\label{fig:interpreter-pipeline}
\end{figure}
